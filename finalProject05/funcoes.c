...
// Função para criar usuário gerente padrão
void criarGerentePadrao() {
    FILE *arquivo = fopen("H:\\ACADEMIC\\CODES\\C\\Gerenciamento_Estoque_C\\finalProject05\\usuarios.txt", "r");  // Abre o arquivo "usuarios.txt" no modo de leitura
    char linha[256];  // Variável para armazenar cada linha lida do arquivo
    int gerenteExiste = 0;  // Variável que indica se o gerente padrão já existe no arquivo
    if (arquivo != NULL) {  // Verifica se o arquivo foi aberto com sucesso
        while (fgets(linha, sizeof(linha), arquivo)) {  // Lê cada linha do arquivo até o final
            if (strstr(linha, "12345678909")) {  // Verifica se a linha contém o CPF do gerente
                gerenteExiste = 1;  // Define que o gerente já existe
                break;  // Sai do loop, pois o gerente já foi encontrado
            }
        }
        fclose(arquivo);  // Fecha o arquivo após terminar de ler
    }
    if (!gerenteExiste) {  // Se o gerente não existir, cria o cadastro
        arquivo = fopen("H:\\ACADEMIC\\CODES\\C\\Gerenciamento_Estoque_C\\finalProject05\\usuarios.txt", "a");  // Abre o arquivo "usuarios.txt" no modo de adição (append)

        if (arquivo != NULL) {  // Verifica se o arquivo foi aberto com sucesso para escrita
            fprintf(arquivo, "12345678909 12345678\n");  // Escreve os dados do gerente padrão no arquivo
            fclose(arquivo);  // Fecha o arquivo após a escrita
            printf("Gerente padrao criado com sucesso.\n");  // Imprime uma mensagem de confirmação
        } else {
...
int validacaoExataCpf(char* cpf) {
...
int validarDados(const char* cpf, const char* senha) {
    ...
    // Aqui você pode adicionar uma função para validar o CPF de forma mais rigorosa
    if (!validacaoExataCpf(cpf)) {
        printf("\nCPF invalido.\n");
        return 0; // Dados inválidos
    }
...
int carregarUsuarios(Usuario* usuarios) {
    FILE* file = fopen("H:\\ACADEMIC\\CODES\\C\\Gerenciamento_Estoque_C\\finalProject05\\usuarios.txt", "r");
    if (!file) {
...
void salvarUsuarios(Usuario* usuarios, int numUsuarios) {
    FILE* file = fopen("H:\\ACADEMIC\\CODES\\C\\Gerenciamento_Estoque_C\\finalProject05\\usuarios.txt", "w");
    if (!file) {
...
int login(Usuario* usuarios, int numUsuarios) {
    ...
    if (!validarDados(cpf, senha)) {
        return 0; // Dados inválidos, encerra a função
    }
...
void cadastrarUsuario(Usuario* usuarios, int* numUsuarios) {
    ...
    while (1) { // Loop para permitir repetição até entrada válida
        ...
        if (codUsuario == codAcesso) {
            ...
            // Validação de dados
            if (!validarDados(cpf, senha)) {
                printf("Dados invalidos. CPF deve ter 11 dígitos, ser existente e senha ter 8 caracteres.\n");
